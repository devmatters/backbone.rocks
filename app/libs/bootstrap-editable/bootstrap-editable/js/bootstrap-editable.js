...
(function ($) {
    ...
    // jx: popover 裏真正繪出 form 供用戶操作的地方
    // 注意 $div 就是指向這個 form 
    ...
    // jx: form 的 init 階段，要鋪初始資料應該也是這時下手
    EditableForm.prototype = {
        ...
        **/        
        ...
        // jx: 處理存檔的地方 #EditableForm.prototype
        submit: function(e) {
            e.stopPropagation();
            e.preventDefault();
    ...
    jx: 這裏開始掛入 jquery 內成為 plugin ← 但注意這只是 EditableForm，是可編輯的表單
    稍後才會掛入 $.fn.Editable，那是要處理頁面上 html elm 的地方
...
(function ($) {
    ...
    // jx: 這裏定義了 x-editable 是 popup or inline 呈現，預設是 popup
    ...
    // jx: Popup 是個虛擬物件，主掌建立 container, form 等各子元素，也保留 $element 指向 html elm
    //methods
    Popup.prototype = {
        ...
        */         
        ...
        // jx: 最終會到這裏，轉手操作 bootstrap 開啟 popover
        call: function() {
        ...
        renderForm: function() {
            ...
            .on({
                // jx: 這裏針對內部 form 掛上一堆偵聽，後來 this.$div.triggerHandler('save') 就是在觸發這裏的 method
    ...
    jx: 這是最外層 popup container, 其實就是內包 bootstrap popover 元件
    **/  
...
jx: 主戲上場，將頁面上選定的 html elm 套上 x-editable plugin
**/
(function ($) {
    "use strict";

    var Editable = function (element, options) {
        // jx: $element 指的是頁面上的 html elm
        this.$element = $(element);
    ...
    Editable.prototype = {
        ...
        init: function () {
            ...
            $.when(doAutotext ? this.render() : true).then($.proxy(function() {
               ...
               $('#username').editable();
               **/                 
               // jx: 這種都是越位代替 html elm 廣播事件，讓外界可偵聽並處理 
        ...
        **/  
        ...
        show: function (closeAll) {
            ...
            if(!this.container) {
                ...
                //listen `save` event 
                // jx: 注意這裏的 save.internal 他只是利用 event namespace 跟一般的 save 事件區隔
                // 因為稍後他需要只 off( 'save.internal' ) ←另方面來說這是在 abuse jquery event namespace 功能
                // 總之它是偷掛在頁面 html elm 上一起聽 save 事件，然後做事
        ...
        */          
        // jx: #Editable methods
        // popup submit 後內部跑完 save()，最後會跑這個 save()，主要功能是去更新頁面上 html elm 的值
        save: function(e, params) {
            ...
            // jx: #Editable methods
            // jx: 這裏將編輯過的值返填回原本文字框，會呼叫 select.value2HtmlFinal()            
            //set new value
            // jxadded: 加上判斷外來參數，以決定 x-editable 是否要回填 html elem value
            // 目地是當使用 select2 時，我可能會由外面接手處理 save() 後續事宜，而不用它自動改值
            // if( this.options.hasOwnProperty('skipSetValue') && this.options.skipSetValue == false )
            if( this.options.skipSetValue == true )
                return;
    ...
    AbstractInput.prototype = {
       ...
       **/       
       ...
       /**
        ...
        jx: 將編輯過的新值填回 html elem，有些元件會進一步實作 value2htmlFinal() 做最後期處理
       **/       
       ...
       **/       
       ...
       /**
        ...
        jx: 這是 Abstract class 定義的基底，但各種 input 可能會覆寫
        @method input2value() 
       **/         
...
(function ($) {
    ...
    // jx: 注意 Select inherit List，因此這裏的 method 也會在 select 被呼叫
    ...
    $.extend(List.prototype, {
         ...
         * jxadded
         *
         * List, Select 需要客製化來取值，因為 value 是 model 物件
         */
        // input2value: function() { 
        ...
        // Select|List 存檔時，用此將選取值轉換為 string 以供顯示
...
(function ($) {
    ...
    // jx: select2 同樣繼承所有 Abstract methods
...
(function ($) {
    ...
    // jx: 有趣, 這是他實現 Abstract class 的方式
