...
Backbone.Paginator = (function ( Backbone, _, $ ) {
  ...
  Paginator.clientPager = Backbone.Collection.extend({
     ...
     * jx: clientPager
     *
     * collection.fetch( options ) → sync() 進到這裏
     *
     * options{} 裏包了一個參數物件 {data: {name:''}}，
     * 指定了要查詢的人名，也就是下面 sync() 中的第三個參數
     */
    sync: function ( method, model, options ) {
      var self = this;

      // jxadded - clientPager 每次 fetch() 時都要清空舊資料，因為撈回來的新資料可能筆數完全不一樣
      // 因此要清掉它之前 cache 住的資料
      ...
      // 如果前面從 paginator_core{} 裏撈不到資料，這裏補上預設值
      ...
      // 注意原本透過 fetch(options) 傳進來的參數，在這裏併入了 queryOptions
      // 更重要的是，server_api{} 裏的參數，編碼後放入 data 屬性，將來傳回 server 是那邊真正要看的
      ...
      // jx: ↑ 到這裏所有需發送給 server 的參數都準備好了，下面準備送出 xhr 
      ...
      // jxadded: 如果我的 collection 要提供自已的 sync method，例如要操作自製的 MemoryStore，就用這個名字
      if( this.sync2 ){
        this.sync2.call( this, queryOptions );
      }else{
        // 這裏真正送出去，有回應時，前面定義的 success callback 就會處理
        var xhr = queryOptions.xhr = $.ajax( queryOptions );
      }

      // 下面只是在越位幫忙在 collection 身上廣播出 request 事件，說不定外界有人在聽
      if ( model && model.trigger ) {
    ...
    info: function () {
      ...
      //jxadded: 廣播告知外界分頁資料有變，後來因為造成循環事件觸發，因此暫不用
