...
define("inverted/ProtoFactory", ["inverted/DependencyTree", "inverted/Util"], function(DependencyTree, Util) {
    ...
    ProtoFactory.prototype.getProto = function(protoRef, depTree) {
        ...
        } else if ((!protoConf.scope || protoConf.scope !== "singleton") || (protoConf.scope === "singleton" && !protoConf.instance)) {

            var injectAppContext = this.injectAppContext === true && protoConf.injectAppContext !== false || this.injectAppContext !== true && protoConf.injectAppContext === true;
    ...
    ProtoFactory.prototype._createInstance = function(protoId, protoConf, interfaces, injectAppContext, depTree) {
        ...
        if (protoConf.mixin && protoConf.mixin.length) {
            ...
            var i, len = mixin.length,
                currentMixin, mixinRef, override;
            for (i = 0; i < len; i++) {
 ...
 * }
 */
define("inverted/AppContext", ["inverted/ProtoFactory", "inverted/DependencyTree", "inverted/Promise", "inverted/Util"],
...
function(ProtoFactory, DependencyTree, Promise, Util) {
    ...
    AppContext.prototype._getDependencies = function(id, depTree) {
        ...
        if (protoConfig.mixin && protoConfig.mixin.length) {
            var i, len = protoConfig.mixin.length,
                currentMixin, mixinRef;
            for (i = 0; i < len; i++) {
